(function(window){
    ...
    for (var i = 1; i <= 7; i++) {
        var img = new Image();
        img.isValid = false;
        img.onload = function() { this.isValid = true; };
        img.onerror = function() { this.isValid = false; };
 ...
 Seed.prototype = {
        ...
        drawHeart: function() {
            ...
            if (img && img.isValid && img.complete && img.naturalWidth > 0) {
                ctx.drawImage(img, -w / 2, -h / 2, w, h);
            } else {
                // fallback: flor rosa clara en vez de amarillo
                var petalColor = 'rgba(255, 105, 180, 0.7)';
                var coreColor = 'rgba(255, 182, 193, 0.9)';
                var pSize = 8 * scale;

                for (var i = 0; i < 10; i++) {
                    var angle = (Math.PI * 2 / 10) * i;
                    var px = Math.cos(angle) * pSize * 1.2;
                    var py = Math.sin(angle) * pSize * 1.2;
                    ctx.beginPath();
                ...
                ctx.arc(0, 0, pSize * 0.8, 0, Math.PI * 2);
                ctx.fill();
            }
        ...
        drawText: function() {
            ...
            ctx.fillText("  Peonías Para Tí", 23, 16); 
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
    ...
    Tree.prototype = {
        ...
        snapshot: function(k, x, y, width, height) {
            ...
            try {
                ...
                console.warn('Canvas tainted by CORS, creating empty ImageData');
    ...
    Bloom.prototype = {
        ...
        draw: function() {
            ...
            if (img && img.isValid && img.complete && img.naturalWidth > 0) {
                ...
                // Fallback: pétalo suave en vez de amarillo fuerte
            ...
            // Estilo original: solo imagen / pétalo de fallback, sin contorno extra
            ctx.restore();
        },
        jump: function() {
